...
DBP_COLOR = "#cc7700"
RATIO_COLOR = "#4b3f9f"
PASSBAND_COLOR = "#e3b341"
HEART_BAND_COLOR = "#36a269"
GRID_CLR = "#e8e8ee"
...
class PSDBrowser:
    ...
    def __init__(self, root: tk.Tk, dataset_dir: Path, target_hz: int, nperseg: int):
        self.root = root
    ...
    def _start_metadata_worker(self) -> None:
        ...
        self._metadata_thread = threading.Thread(
            ...
            name="psd-browser-metadata-loader",
            daemon=True,
        )
    ...
    def _build_canvas_panel(self, parent: tk.Frame) -> None:
        ...
        self._fig = plt.Figure(figsize=(9.2, 6.5), facecolor=BG_DARK)
        self._gs = gridspec.GridSpec(2, 1, figure=self._fig, height_ratios=[1.1, 1.0])
        self._ax_wave = self._fig.add_subplot(self._gs[0], facecolor=BG_DARK)
        self._ax_psd = self._fig.add_subplot(self._gs[1], facecolor=BG_DARK)
    ...
    def _compute_case_ratio_cache(self, path: Path) -> None:
        ...
        for signal in self._x:
            _, _, _, _, ratio = compute_ratio_stats(signal, self.target_hz, self.nperseg)
            ratios.append(ratio)
    ...
    def _update_ratio_results(self) -> None:
        ...
        for seg_idx in self._ratio_match_indices:
            ...
            self._ratio_results.insert(
                tk.END,
                f"Seg {seg_idx + 1:4d}   ratio {ratio:0.4f}",
            )

        if self._ratio_match_indices:
            self._ratio_status.set(
                f"{len(self._ratio_match_indices)} segments in range {label}"
            )
        else:
    ...
    def _show_segment(self, idx: int) -> None:
        ...
        freqs, psd, heart_power, passband_power, ratio = compute_ratio_stats(
            ppg,
            self.target_hz,
            self.nperseg,
        )
        ...
        ax_psd.plot(freqs, psd, color=PSD_COLOR, linewidth=1.4, label="PSD")
        ax_psd.axvspan(PASSBAND[0], PASSBAND[1], color=PASSBAND_COLOR, alpha=0.16, label="0.5-10.0 Hz")
        ax_psd.axvspan(HEART_BAND[0], HEART_BAND[1], color=HEART_BAND_COLOR, alpha=0.22, label="0.67-3.0 Hz")
        ax_psd.set_xlim(0, min(10.5, self.target_hz / 2))
        ax_psd.set_title("Welch Power Spectral Density", color=FG_NORM, fontsize=11)
        ax_psd.set_xlabel("Frequency (Hz)", color=FG_DIM, fontsize=9)
        ax_psd.set_ylabel("PSD", color=FG_DIM, fontsize=9)
        ax_psd.legend(loc="upper right", fontsize=8, frameon=True)
        ...
        if ratio_stats is not None and len(ratio_stats) > 0:
            mean_ratio = float(np.nanmean(ratio_stats))
            ax_psd.text(
                0.02,
                0.97,
                f"Case mean ratio {mean_ratio:.4f}",
                transform=ax_psd.transAxes,
                ha="left",
                va="top",
                color=FG_DIM,
...
def main() -> None:
    ...
    root = tk.Tk()
    PSDBrowser(root, args.dataset_dir, args.target_hz, args.nperseg)
    root.mainloop()
