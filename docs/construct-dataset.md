...
- `--nproc`
  ...
  - 기본값: `os.cpu_count()`
- `--no-resume`
  - 출력 NPZ가 이미 존재해도 해당 케이스를 재처리한다.
  - 기본 동작(resume=on): 출력 NPZ가 이미 있으면 케이스를 skip한다.
- `--seed`
  - 케이스 shuffle용 난수 시드
  - 기본값: `42`
...
```bash
uv run python scripts/construct-dataset.py --nproc 4
```

중단된 실행을 처음부터 다시 생성 (기존 파일 무시):

```bash
uv run python scripts/construct-dataset.py --no-resume
```
...
### 4.3 병렬 처리 및 Resume 체크

#### 4.3.1 멀티프로세싱

split 분배가 완료되면 세 split의 케이스를 하나의 flat task list로 합친다.
`i::nproc` 슬라이싱으로 `nproc`개의 청크로 균등 분배한 뒤,
각 청크를 독립적인 워커 프로세스에서 병렬 처리한다.

각 워커는 `tqdm`의 `position` 파라미터로 고정된 줄에 개별 progress bar를 출력한다.

```text
  proc  0: 100%|████████| 400/400 [06:12<00:00,  1.07case/s]
  proc  1:  87%|███████  | 348/400 [05:24<00:47,  1.07case/s]
  ...
  proc 15:  91%|████████ | 364/400 [05:38<00:33,  1.07case/s]
```

세 split을 합쳐서 한 번에 분배하므로 특정 split이 먼저 끝나도 idle 워커가 없다.
워커는 NPZ 파일을 직접 저장하고, 세그먼트 수와 skip 수만 집계하여 반환한다.

#### 4.3.2 Resume 체크

Resume 기능(기본값: 활성)은 해당 케이스의 출력 NPZ 파일이 이미 존재하면
`process_case()` 호출 없이 즉시 skip한다.

```text
출력 NPZ 존재?
  예 + resume=on   →  skip (resumed 카운트 증가)
  아니오 또는 no-resume  →  process_case() 실행
```

중단된 실행을 재시작할 때 이미 완료된 케이스를 건너뛸 수 있다.
`--no-resume`을 지정하면 기존 파일 여부와 무관하게 모든 케이스를 재처리한다.
...
Resume 체크(4.3.2)는 이 경로(`out_dir / f"{path.stem}.npz"`)를 기준으로 파일 존재 여부를 판단한다.
...
### `process_case(path, target_hz, segment_sec, guard_sec)`
...
멀티프로세싱 워커 함수다. `mp.Pool`에서 호출되며, 모듈 최상위에 정의되어 있어
`pickle`로 직렬화할 수 있다.

역할:

- 자신의 청크에 속한 케이스를 순회
- Resume 체크: 출력 NPZ 존재 시 skip
- `process_case()` 호출 및 NPZ 저장
- tqdm progress bar(`position=worker_id`)로 진행 상황 출력
- 세그먼트 수, skip 수, resume 수를 split별로 집계하여 반환
...
```text
15:54:31 [INFO] Found 6388 .vital files in data/vitaldb
15:54:31 [INFO] Settings: target_hz=125  segment_sec=8s  overlap=4s  guard=1s  nproc=16  resume=on  split=70/10/20
15:54:31 [INFO]   train : 4471 cases
15:54:31 [INFO]   val   :  638 cases
15:54:31 [INFO]   test  : 1279 cases
  proc  0: 100%|████████| 400/400 [06:12<00:00,  1.07case/s]
  proc  1: 100%|████████| 399/399 [06:08<00:00,  1.08case/s]
  ...
  proc 15: 100%|████████| 399/399 [05:50<00:00,  1.14case/s]
```

split 처리 후에는 다음과 같은 요약이 나온다.

**첫 실행 (resume=on, 기존 파일 없음):**

```text
16:01:14 [INFO]   train done — 123456 segments from 3800 new cases (0 resumed, 671 skipped)
16:01:14 [INFO]   val   done —  18500 segments from  560 new cases (0 resumed, 78 skipped)
16:01:14 [INFO]   test  done —  37200 segments from 1120 new cases (0 resumed, 159 skipped)
```

**중단 후 재실행 (resume=on, 일부 파일 이미 존재):**

```text
16:01:14 [INFO]   train done — 45678 segments from 1000 new cases (2800 resumed, 671 skipped)
16:01:14 [INFO]   val   done —   5200 segments from  160 new cases ( 400 resumed, 78 skipped)
16:01:14 [INFO]   test  done —  10400 segments from  320 new cases ( 800 resumed, 159 skipped)
```

여기서 각 항목의 의미는 다음과 같다.

- `new cases`: 이번 실행에서 새로 처리된 케이스 수
- `resumed`: 출력 NPZ가 이미 존재하여 skip된 케이스 수
- `skipped`: `process_case()`가 `None`을 반환한 케이스 수
  - 필요한 트랙이 없음
  - 파일을 열 수 없음
  - 트랙 읽기 실패
  - 세그먼트가 하나도 남지 않음
...
### 7.4 Resume과 출력 디렉터리 관리

스크립트는 `output-dir/train|val|test`를 생성만 하고, 기존 파일을 먼저 지우지는 않는다.

기본 동작(`resume=on`):

- 출력 NPZ가 이미 존재하면 해당 케이스는 처리하지 않고 skip한다.
- 중단된 실행을 재시작할 때 안전하게 이어서 진행할 수 있다.

`--no-resume` 지정 시:

- 기존 파일 여부와 무관하게 모든 케이스를 재처리한다.
- 같은 케이스 파일명은 새로 덮어써진다.

주의: split 배정이 바뀌어도 이전 split에서 생성된 오래된 NPZ 파일은
자동으로 삭제되지 않는다. 설정을 크게 바꾸어 완전히 새로 만들고 싶다면,
실행 전에 출력 디렉터리를 직접 삭제하는 것이 안전하다.
...
- PPG를 500 Hz에서 목표 주파수로 단순 decimation 후
  4차 버터워스 대역통과 필터(0.5–10 Hz, zero-phase)를 윈도우 단위로 적용한다.
- Guard-band 기법으로 필터 edge artifact를 완화한다(기본 1초).
- 8초 기본 윈도우와 50% overlap으로 세그먼트를 만든다.
- 1 Hz BP 수치의 구간 평균으로 `SBP`, `DBP` 레이블을 만든다.
- 품질이 낮거나 생리적으로 이상한 구간은 적극적으로 제거한다.
- `multiprocessing.Pool`로 케이스를 병렬 처리하며, 워커마다 progress bar를 출력한다.
- Resume 기능(기본 활성)으로 중단된 실행을 안전하게 이어서 진행할 수 있다.
