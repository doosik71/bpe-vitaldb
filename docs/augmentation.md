...
적용 실험: [evaluation_result.md](evaluation_result.md)  
비교 기준: [evaluation_result_no_aug.md](evaluation_result_no_aug.md)
...
**클래스:** `AmplitudeScaling(lo=0.9, hi=1.1)`  
...
| 파라미터 | 기본값 | 의미 |
| -------- | ------ | ---- |
| `lo`     | 0.9    | 스케일 하한 (원본의 90%) |
| `hi`     | 1.1    | 스케일 상한 (원본의 110%) |

#### 설계 근거

z-score 정규화 후에 적용하므로 평균은 ~0으로 유지되고 분산만 ±10% 범위에서 섭동된다.
PPG 센서의 접촉 압력 차이나 피부 색소 농도에 따른 신호 이득 변화를 모사한다. 스케일
범위 ±10%는 정규화 신호의 형태 정보를 보존하면서 이득 불변성을 학습시키기에 적합한
범위로 설정됐다.
...
| 파라미터    | 기본값 | 의미 |
| ----------- | ------ | ---- |
| `max_shift` | 25     | 최대 이동량 (샘플 수), 125 Hz 기준 ±0.2초 |

#### 설계 근거

8초 세그먼트(1000 샘플) 내에서 최대 ±25샘플(±0.2초) 이동은 세그먼트 길이의 ±2.5%에
...
세그먼트에서 무작위로 선택한 샘플들을 0으로 대체한다. 마스킹 비율 `p`는 매 호출 시
균일 분포에서 샘플링된다. 마스킹 위치는 비복원 추출(without replacement)로 선정된다.

```
p ~ Uniform(lo_frac, hi_frac)
n_mask = max(1, floor(N · p))
indices = randperm(N)[:n_mask]        (비복원 추출)
x_aug[indices] = 0.0                   (z-score 정규화 후 평균값)
```
...
| 파라미터   | 기본값 | 의미 |
| ---------- | ------ | ---- |
| `lo_frac`  | 0.05   | 마스킹 비율 하한 (5%) |
| `hi_frac`  | 0.10   | 마스킹 비율 상한 (10%) |

#### 설계 근거

마스킹값 0.0은 z-score 정규화된 신호의 평균값과 동일하여 마스킹이 평균 보간과 동등하게
처리된다. 마스킹 위치가 **산포(scatter)** 방식으로 선택되므로 연속 구간 손실이 아닌
점별 결측 패턴을 모사한다. 5~10% 범위는 임상 환경의 동작 아티팩트나 신호 포화로 인한
단편적 손실을 반영한다. MTAE 계열 모델에서 특히 재구성 손실과 시너지 효과가 예상된다.
...
if args.aug_scale:  aug_transforms.append(AmplitudeScaling(lo=0.9, hi=1.1))
if args.aug_shift:  aug_transforms.append(TimeShift(max_shift=25))
...
| `--no-aug-mask`   | Random Masking 비활성화             |
...
상세 결과는 [evaluation_result.md](evaluation_result.md) 참조.
