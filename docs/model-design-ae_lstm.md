...
입력: PPG 세그먼트
    ...
    ┌─────────────────┴──────────────────────────┐
    │  _LSTMEncoder                              │
    │  LSTM(input_size=1, hidden_size=100)       │
    │    → 출력: (B, 1000, 100)  [all timesteps] │
    │    → 은닉: h_n (1, B, 100) [최종 은닉 상태]│
    │  h_n.squeeze(0)  → (B, 100)                │
    │  Dropout(0.2)    → (B, 100)                │
    └─────────────────┬──────────────────────────┘
              (B, 100)   ← 잠재 벡터 z
 ...
 Linear(100→32) → ReLU → Linear(32→2) ┌─────────┴─────────────────────────────┐
        │                             │  _LSTMDecoder                         │
     (B, 2)                           │  z.unsqueeze(1)  → (B, 1, 100)        │
  [SBP, DBP]                          │  expand(-1, 1000, -1) → (B, 1000, 100)│
                                      │  LSTM(input_size=100, hidden_size=100)│
                                      │    → (B, 1000, 100)                   │
                                      │  Linear(100→1)  → (B, 1000, 1)        │
...
| 2    | LSTM(input=1, hidden=100, batch_first) | (B, 1000, 1) | out: (B, 1000, 100), h_n: (1, B, 100) |
| 3    | h_n.squeeze(0)                         | (1, B, 100)  | (B, 100)                              |
| 4    | Dropout(0.2)                           | (B, 100)     | (B, 100)                              |

### _LSTMDecoder

| 단계 | 처리                                     | 입력 shape     | 출력 shape     |
| ---- | ---------------------------------------- | -------------- | -------------- |
| 0    | unsqueeze(1)                             | (B, 100)       | (B, 1, 100)    |
| 1    | expand(-1, 1000, -1).contiguous()        | (B, 1, 100)    | (B, 1000, 100) |
| 2    | LSTM(input=100, hidden=100, batch_first) | (B, 1000, 100) | (B, 1000, 100) |
| 3    | Linear(100→1)                            | (B, 1000, 100) | (B, 1000, 1)   |
...
| 단계 | 처리           | 입력 shape | 출력 shape |
| ---- | -------------- | ---------- | ---------- |
| 0    | Linear(100→32) | (B, 100)   | (B, 32)    |
| 1    | ReLU           | (B, 32)    | (B, 32)    |
| 2    | Linear(32→2)   | (B, 32)    | (B, 2)     |
...
(맥박 진폭, 파형 형태, 상승/하강 기울기 등)을 100차원 잠재 벡터로 압축한다.
...
# h_n: (num_layers=1, B, hidden_size=100)
return self.drop(h_n.squeeze(0))   # (B, 100)
```
...
| hidden_size | 100                                                  |
| 파라미터 수 | 4 × 100 × (1 + 100 + 2) = 4 × 100 × 103 = **41,200** |
...
# z:     (B, 100)
# z_seq: (B, 1000, 100) — 같은 잠재 벡터를 1000번 반복
h, _ = self.lstm(z_seq)   # (B, 1000, 100)
```
...
| 항목          | 값                                                     |
| ------------- | ------------------------------------------------------ |
| input_size    | 100 (잠재 벡터 차원)                                   |
| hidden_size   | 100                                                    |
| LSTM 파라미터 | 4 × 100 × (100 + 100 + 2) = 4 × 100 × 202 = **80,800** |
| out_proj      | Linear(100→1): 100 × 1 + 1 = **101**                   |
...
이 구현에서는 VitalDB의 연속적 SBP/DBP 레이블에 맞게 **2-layer MLP 회귀 헤드**로
변경한다. 첫 번째 선형층은 100차원 잠재 표현을 32차원 중간 표현으로 압축하고,
ReLU 비선형성을 거쳐 최종 SBP/DBP를 회귀한다.

파라미터 수: (100 × 32 + 32) + (32 × 2 + 2) = **3,298**

## 5. 파라미터 수 분석

| 모듈             | 구성                   | 파라미터 수 |
| ---------------- | ---------------------- | ----------- |
| encoder.lstm     | LSTM(1, 100)           | 41,200      |
| encoder.drop     | Dropout(0.2)           | 0           |
| decoder.lstm     | LSTM(100, 100)         | 80,800      |
| decoder.out_proj | Linear(100, 1)         | 101         |
| bp_head          | Linear(100, 32) + ReLU | 3,298       |
| **합계**         |                        | **125,399** |

```text
print-model 출력:
  Total params    : 125,399  (125.4 K)
  Trainable params: 125,399  (125.4 K)
  Input shape     : (1, 1000)
```

디코더 LSTM이 전체의 64.4%를 차지하며, 인코더 LSTM이 32.9%를 차지한다.
BP 헤드와 출력 투영을 합쳐도 약 2.7%에 불과하다.
...
이 구현은 hidden_size=100으로 설정해 각 PPG 샘플(스칼라)을 LSTM의 1차원 입력으로
직접 처리하며, 총 파라미터 수는 125,399(≈ 125.4 K)이다.
...
- 기본값 `recon_weight=0.2`: BP 회귀를 우선하고, 재구성은 보조 목표로만 반영
...
| 파라미터       | 기본값 | 논문 설정 | 역할                                      |
| -------------- | ------ | --------- | ----------------------------------------- |
| `hidden_size`  | 100    | 100       | LSTM 인코더/디코더 은닉 상태 차원         |
| `dropout`      | 0.2    | 0.2       | 인코더 출력 드롭아웃 비율 (과적합 방지)   |
| `seq_len`      | 1000   | (가변)    | 입·출력 시퀀스 길이 (8 s × 125 Hz)        |
| `recon_weight` | 0.2    | 미명시    | 재구성 손실 가중치 (BP 회귀 우선)         |
...
SBP/DBP 수치이므로, 이 구현은 **Linear(100→32) → ReLU → Linear(32→2) + MSE/Huber 손실**로
회귀를 수행한다.
...
PPG 재구성을 보조 자기지도 목표로 추가한다. 기본값은 `recon_weight=0.2`로 두어
혈압 추정을 우선하고, 재구성은 정규화 역할에 머물도록 했다. `recon_weight=0.0`으로
설정하면 재구성 없이 순수 BP 회귀만 수행한다.
...
### 8.6 hidden_size: 논문 값 100 채택

논문 Table 1은 "No. of Hidden units-100"으로 기술한다. 이 구현도 이에 맞춰
**hidden_size=100**을 사용하며, 인코더와 디코더가 동일한 100차원 잠재 공간을
공유한다. 현재 구성의 총 파라미터 수는 **125,399**이다.
...
--------------------------------------------------------------------------------------
encoder            _LSTMEncoder   (1, 100)
encoder.lstm       LSTM           (1, 1000, 100) 41.2 K
encoder.drop       Dropout        (1, 100)
decoder            _LSTMDecoder   -
decoder.lstm       LSTM           -              80.8 K
decoder.out_proj   Linear         -                 101
bp_head            Sequential     (1, 2)          3.3 K
  bp_head.0        Linear         (1, 32)         3.2 K
  bp_head.1        ReLU           (1, 32)             0
  bp_head.2        Linear         (1, 2)             66
--------------------------------------------------------------------------------------
  Total params    : 125,399  (125.4 K)
  Trainable params: 125,399  (125.4 K)
  Input shape     : (1, 1000)
...
| `ae_lstm`       | LSTM 인코더  | LSTM 디코더         | 있음 (LSTM) | 125.4 K  |
...
CNN이나 Attention 없이 시계열 의존성만으로 BP를 추정한다. 파라미터 수는 125.4 K로
비교적 작은 편이며, LSTM의 순차적 처리 특성상 순수 CNN 모델보다 배치당 처리 시간이 길다.
